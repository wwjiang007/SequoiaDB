...
function updateTaskInfo( taskInfo, isFirst )
{
	function typeToStr( type, errno )
	{
		...
		if( errno === 0 )
		{
			...
			{
				//'正在初始化任务'
				str = _languagePack['uninstzookeeper']['rightPanel']['taskStatus'][0] ;
			}
			else if( type === 1 )
			{
				//'正在执行任务'
				str = _languagePack['uninstzookeeper']['rightPanel']['taskStatus'][1] ;
			}
			else if( type === 2 )
			{
				//'正在回滚任务'
				str = _languagePack['uninstzookeeper']['rightPanel']['taskStatus'][2] ;
			}
			else if( type === 3 )
			{
				//'正在取消任务'
				str = _languagePack['uninstzookeeper']['rightPanel']['taskStatus'][3] ;
			}
			else if( type === 4 )
			{
				//'任务完成'
				str = _languagePack['uninstzookeeper']['rightPanel']['taskStatus'][4] ;
			}
			else
			{
				//'未知状态'
				str = _languagePack['uninstzookeeper']['rightPanel']['taskStatus'][5] ;
			}
		}
		else
		{
			//'任务失败'
			str = _languagePack['uninstzookeeper']['rightPanel']['taskStatus'][6] ;
		}
		return str ;
	}
	function typeToStr2( type, errno )
	{
		...
		if( errno === 0 )
		{
			...
			{
				//'正在初始化卸载'
				str = _languagePack['uninstzookeeper']['rightPanel']['nodeStatus'][0] ;
			}
			else if( type === 1 )
			{
				//'正在卸载'
				str = _languagePack['uninstzookeeper']['rightPanel']['nodeStatus'][1] ;
			}
			else if( type === 2 )
			{
				//'正在回滚卸载'
				str = _languagePack['uninstzookeeper']['rightPanel']['nodeStatus'][2] ;
			}
			else if( type === 3 )
			{
				//'正在取消卸载'
				str = _languagePack['uninstzookeeper']['rightPanel']['nodeStatus'][3] ;
			}
			else if( type === 4 )
			{
				//'卸载完成'
				str = _languagePack['uninstzookeeper']['rightPanel']['nodeStatus'][4] ;
			}
			else
			{
				//'未知状态'
				str = _languagePack['uninstzookeeper']['rightPanel']['nodeStatus'][5] ;
			}
		}
		else
		{
			//'卸载失败'
			str = _languagePack['uninstzookeeper']['rightPanel']['nodeStatus'][6] ;
		}
		return str ;
	}
	var errNode = 0 ;
	...
	{
		//'分钟'
		sdbjs.parts.tableBox.updateBody( 'nodeInfoTable', 6, 1, htmlEncode( remainTime + _languagePack['uninstzookeeper']['leftPanel']['time'] ) ) ;
	}
	else
	{
	...
	$.each( taskInfo['ResultInfo'], function( index, resultInfo ){
		if( _nodeStatus[index] === false )
		{
			...
			sdbjs.parts.gridBox.updateBody( 'nodeListGrid', index, 3, htmlEncode( statusStr ) ) ;
			sdbjs.parts.gridBox.updateBody( 'nodeListGrid', index, 4, htmlEncode( typeToStr2( resultInfo['Status'], resultInfo['errno'] ) ) ) ;
		}
		if( resultInfo['errno'] !== 0 )
		{
			++errNode ;
...
function createHtml()
{
	...
	//'任务信息'
	sdbjs.parts.panelBox.update( 'nodeInfoBar', htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['title'] ), function( panelBody ){
		sdbjs.parts.tableBox.create( panelBody['name'], 'nodeInfoTable' ) ;
		sdbjs.parts.tableBox.update( 'nodeInfoTable', 'loosen border' ) ;
		//'任务ID'
		sdbjs.parts.tableBox.addBody( 'nodeInfoTable', [ { 'text': htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['taskInfo'][0] ), width: 150 }, { 'text': '' } ] ) ;
		//'所属集群'
		sdbjs.parts.tableBox.addBody( 'nodeInfoTable', [ { 'text': htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['taskInfo'][1] ) }, { 'text': '' } ] ) ;
		//'任务类型'
		sdbjs.parts.tableBox.addBody( 'nodeInfoTable', [ { 'text': htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['taskInfo'][2] ) }, { 'text': '' } ] ) ;
		//'项目数'
		sdbjs.parts.tableBox.addBody( 'nodeInfoTable', [ { 'text': htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['taskInfo'][3] ) }, { 'text': '' } ] ) ;
		//'任务状态'
		sdbjs.parts.tableBox.addBody( 'nodeInfoTable', [ { 'text': htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['taskInfo'][4] ) }, { 'text': '' } ] ) ;
		//'卸载进度'
		sdbjs.parts.tableBox.addBody( 'nodeInfoTable', [ { 'text': htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['taskInfo'][5] ) }, { 'text': '' } ] ) ;
		//'预计剩余时间'
		sdbjs.parts.tableBox.addBody( 'nodeInfoTable', [ { 'text': htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['taskInfo'][6] ) }, { 'text': '' } ] ) ;
		
		//'日志'
		sdbjs.parts.tableBox.addBody( 'nodeInfoTable', [ { 'text': htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['taskInfo'][7] ) }, { 'text': '<button class="btn btn-default btn-lg" onclick="openLogModal()">' + htmlEncode( _languagePack['uninstzookeeper']['leftPanel']['logButton'] ) + '</button>' } ] ) ;
	} ) ;
	...
	//'卸载进度'
	sdbjs.parts.panelBox.update( 'installInfoBar', htmlEncode( _languagePack['uninstzookeeper']['rightPanel']['title'] ), function( panelBody ){
		...
		//'主机名' '端口' '卸载状态' '卸载进度' '日志'
		sdbjs.parts.gridBox.addTitle( 'nodeListGrid', [{ 'text': '', 'width': '15%' },
                                                       { 'text': htmlEncode( _languagePack['uninstzookeeper']['rightPanel']['nodeGrid'][0] ), 'width': '25%' },
                                                       { 'text': htmlEncode( _languagePack['uninstzookeeper']['rightPanel']['nodeGrid'][1] ), 'width': '15%' },
                                                       { 'text': htmlEncode( _languagePack['uninstzookeeper']['rightPanel']['nodeGrid'][2] ), 'width': '30%' },
                                                       { 'text': htmlEncode( _languagePack['uninstzookeeper']['rightPanel']['nodeGrid'][3] ), 'width': '15%' } ] ) ;
	...
	//'日志'
	sdbjs.parts.modalBox.update( 'logModal', htmlEncode( _languagePack['uninstzookeeper']['logModal']['title'] ), function( bodyObj ){
