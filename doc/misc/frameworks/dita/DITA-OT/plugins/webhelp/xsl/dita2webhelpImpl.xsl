...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  exclude-result-prefixes="File XSLTExtensionIOUtil oxygen">
        ...
        <span class="navheader_label">
          <xsl:choose>
            <xsl:when test="@role = 'parent'">
              <xsl:value-of select="oxygen:getLocalization('Parent topic')"/>
            </xsl:when>
            <xsl:when test="@role = 'previous'">
              <xsl:value-of select="oxygen:getLocalization('Previous topic')"/>
            </xsl:when>
            <xsl:when test="@role = 'next'">
              <xsl:value-of select="oxygen:getLocalization('Next topic')"/>
            </xsl:when>
          </xsl:choose>
        </span>
...
//]]></xsl:comment>
    ...
    <div class="footer">
    	    <a href="http://www.sequoiadb.com" target="_blank">
    	    	<img src="{$PATH2PROJ}assets/images/sequoiadb_logo.png" alt="SequoiaDB" /></a>SequoiaDB Version 1.0
          <!-- <xsl:value-of select="oxygen:getLocalization('Output generated by')"/>
  ...
  <xsl:template match="/|node()|@*" mode="gen-user-scripts">
    <script type="text/javascript" src="{$PATH2PROJ}assets/jquery-1.3.2.js"><xsl:text> </xsl:text></script>
    ...
    var prefix = "]]></xsl:text>
      ...
      <xsl:variable name="pathWithSlashes" select="translate($differencePath, '\\', '/')"/>
      <xsl:variable name="subdirCount" select="count(tokenize($pathWithSlashes, '/')) - 2"/>
...
//]]></xsl:text>
  ...
  <xsl:template name="getMeta">
      ...
      <xsl:otherwise>
          ...
          <xsl:variable name="bodyTextStart">
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:variable name="description" select="XSLTExtensionIOUtil:getSubstringBeforeLastSpaceOrDot(substring($bodyText, 1, 201))"/>
    ...
    <!-- CONTENT: Subject - indexterms -->
    <xsl:apply-templates select="." mode="gen-indexterms-metadata"/>
  ...
  <!-- OXYGEN PATCH START EXM-20576 -->
  <xsl:template match="*" mode="gen-indexterms-metadata">
    <xsl:variable name="indexterms-content">
      <xsl:for-each select="descendant::*[contains(@class,' topic/indexterm ')]">
