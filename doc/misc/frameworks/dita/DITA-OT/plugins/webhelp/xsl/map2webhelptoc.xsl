...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="generate-toc">
        ...
        <xsl:if test="$CUSTOM_WEBHELP_SEARCH != 'yes'">
          ...
          <div id="searchDiv">
            <form name="searchForm" action="javascript:void(0)" onsubmit="Verifie('searchForm');">
              <xsl:value-of select="oxygen:getLocalization('Keywords')"/>
              <xsl:text>: </xsl:text>
              <xsl:comment> </xsl:comment>
              <input type="text" id="textToSearch" name="textToSearch" class="textToSearch" size="30"/>
              <xsl:comment> </xsl:comment>
              <input type="button" value="{oxygen:getLocalization('Search')}" name="searchButton" class="searchButton"
    ...
    <xsl:result-document doctype-public="-//W3C//DTD XHTML 1.0 Frameset//EN" 
      ...
      href="{concat('index', $OUTEXT)}" indent="yes" encoding="UTF-8" 
      ...
      <html>
        ...
        <frameset cols="25%,*" onload="frames.contentwin.location = getHTMLPage()"> 
          <frame name="tocwin" id="tocwin" src="{$TOC_FILE_NAME}"/>
          <frame name="contentwin" id="contentwin" src="{($toc/ul[1]//li/a[@href])[1]/@href}"/>
        </frameset>
      </html>
    </xsl:result-document>
    <!-- OXYGEN PATCH END EXM-18337 -->
...
//]]></xsl:comment>
  ...
  <xsl:template match="/|node()|@*" mode="gen-user-scripts">
    <!-- OXYGEN PATCH START EXM-17248 -->
    <script type="text/javascript" src="{$PATH2PROJ}assets/jquery-1.3.2.js">
      <xsl:comment></xsl:comment>
    </script>
    ...
    <xsl:call-template name="generateLocalizationJavascript"/>
    <xsl:variable name="javaScriptFileName" select="concat('assets/localization/', $LANGUAGE, '.js')"/>
    <script src="{$javaScriptFileName}" type="text/javascript">
      <xsl:comment></xsl:comment>
    </script>
